// src/app/[locale]/confirm-username/page.tsx
...
import { ConfirmUsernameForm } from "./(components)/confirm-username-form";

export default async function ConfirmUsernamePage({
  params,
  searchParams,
}: {
  ...
  return (
    <div className="min-h-[100dvh] bg-background flex flex-col justify-center items-center px-4 sm:px-6 py-6 sm:py-8 overflow-hidden">
      <ConfirmUsernameForm token={token ?? ""} locale={locale} />
      ...
      <div className="w-full max-w-[1040px] flex flex-col sm:flex-row justify-between items-center mt-4 gap-4">
        ...
        <div className="flex items-center gap-6 text-xs font-semibold text-foreground-60">
          ...
          >
            {t("confirmUsername.help")}
          </Link>
          <Link
            href={`/${locale}/privacy`}
            className="hover:text-foreground transition-colors py-1"
          >
            {t("confirmUsername.privacy")}
          </Link>
        </div>
      </div>
    </div>
