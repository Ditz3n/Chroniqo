// src/components/moderation/extend-mute-modal.tsx
"use client";
...
import { ExtendMuteModalProps } from "@/types/app-types";
import { useEffect, useRef, useState } from "react";

export function ExtendMuteModal({
  isOpen,
  onClose,
  onConfirm,
  targetIdentifier,
  currentExpiration,
}: ExtendMuteModalProps) {
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden gap-0 bg-background border-surface-border">
        <DialogHeader className="bg-surface py-4">
          <DialogTitle className="font-bold">
            {t("admin.extend_mute_title").replace(
              "{{target}}",
              targetIdentifier,
            )}
          </DialogTitle>
          ...
          >
            <div style={gridInnerStyle}>
              ...
              >
                ...
                <Button
                  form="extend-mute-form"
                  type="submit"
                  variant="outline-warning"
                  disabled={isLoading}
                >
                  {isLoading ? t("admin.generating") : t("admin.extend_mute")}
                </Button>
              </div>
            </div>
          </div>
