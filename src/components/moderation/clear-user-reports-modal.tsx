// src/components/moderation/clear-user-reports-modal.tsx
"use client";
...
import { ClearUserReportsModalProps } from "@/types/app-types";
import { useEffect, useRef, useState } from "react";

export function ClearUserReportsModal({
  isOpen,
  onClose,
  onConfirm,
  targetName,
  onSuccessComplete,
}: ClearUserReportsModalProps) {
  ...
  const [descBeforeTarget, descAfterTarget] = t(
    "admin.clear_user_reports_desc",
  ).split("{{target}}");
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden gap-0 bg-background border-surface-border">
        <DialogHeader className="bg-surface py-4">
          <DialogTitle className="font-bold text-foreground">
            {t("admin.clear_user_reports_title").replace(
              "{{target}}",
              targetName,
            )}
          </DialogTitle>
            ...
            <div style={gridInnerStyle}>
              ...
              >
                <p className="text-foreground font-bold text-lg text-center">
                  {t("admin.clear_user_reports_success").replace(
                    "{{target}}",
                    targetName,
                  )}
                </p>
